...
- `2026-07-09`：`x1_dh_stand` 训练初始状态对齐到真机测例 `20260707_163855` 第 0 帧，并在 MuJoCo 推理接触高度基础上降低 1mm：base_z `0.6091938959661087`。
- `2026-07-10`：在上述真机对齐默认关节角基础上启用 reset 关节角扰动；每个环境 reset 时，各关节独立均匀采样 `[-0.1, 0.1] rad` 的偏移。
- `2026-07-10`：每次环境 reset 后增加 10 个 policy step（约 100 ms）的启动等待；第 0～9 帧运动命令和训练奖励保持为 0，第 10 帧启动首个运动命令、重置步态相位并开始计奖。
